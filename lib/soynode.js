 ...
 * @type {string}
 */
var PATH_TO_SOY_JAR = path.join(__dirname, '../node_modules/closure-templates/SoyToJsSrcCompiler.jar')
 ...
 * @type {string}
 */
var PATH_TO_SOY_UTILS = path.join(__dirname, '../node_modules/closure-templates/soyutils.js')
...
var options = {
    ...
    tmpDir: '/tmp/soynode'
    //An output directory, which compiled soy.js files can be stored and accessed after compilation.
  , targetDir: null
...
function setOptions(opts) {
  for (var key in opts) {
    ...
    if (key == 'targetDir') options['targetDir'] = path.resolve(opts['targetDir'])
...
function compileTemplates(inputDir, callback) {
  findFiles(inputDir, 'soy', function (err, files) {
    ...
    //Determine where the compiled soy templates will be saved.
    //If there is a target directory place them there, otherwise place it in a
    //unique location within the tmp directory.
    var outDir = options.targetDir || path.join(options.tmpDir, new Date().toISOString().replace(/\:/g, '_'))
    ...
    cp.on('exit', function (exitCode) {
      if (exitCode != 0) {
        ...
        loadCompiledTemplateFiles(templatePaths, function (err) {
          if (err) return callback(err, false)
          callback(null, true)
          if (!options.targetDir && options.eraseTemporaryFiles) {
