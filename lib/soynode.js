 ...
 * @type {string}
 */
var PATH_TO_SOY = path.join(__dirname, '../soy/SoyToJsSrcCompiler.jar')
...
var soyUtilsPath = path.join(__dirname, '../soy/soyutils.js')
vm.runInNewContext(fs.readFileSync(soyUtilsPath, 'utf8'), vmContext, soyUtilsPath)


/**
 * Sets options which affect how soynode opperates.
 * @param {{
 *     tmpDir: string=,
...
function compileTemplates(inputDir, callback) {
  findFiles(inputDir, 'soy', function (err, files) {
    ...
    var outDir = path.join(options.tmpDir, '/' + new Date().toISOString())

    // Quote the filenames for use in the shell command.
    var quotedFiles = files.map(function (file) { return '\'' + file + '\'' }).join(' ')

    // The command finds all soy files under the cwd and passes them to the compiler.  Files will be
    // output within the outDir with paths that match relative to the inputDir.
    var command = 'java -jar ' + PATH_TO_SOY +
        ' --codeStyle concat ' +
        '--shouldGenerateJsdoc ' +
    ...
    exec(command, {cwd: inputDir}, function (err, stdout, stderr) {
      if (err) {
        // Log all the errors and execute the callback with a generic error object.
        console.error('soynode: Compile error', err)
...
function loadCompiledTemplateFiles(files, callback) {
  function next() {
    if (files.length === 0) {
      ...
      // object.  This can be called mulitple times and will just override references.  This is
  ...
  files.forEach(function (file) {
    ...
    try {
      ...
      fs.watch(file, {}, function (event, filename) {
        var now = Date.now()
        // Ignore suprious change events.
        if (now - watches[file] < 500) return
